...
def generate_graph(trial_id):
    ...
    alpha = zip(*alpha_data)
    beta_data = zip(*beta_data)
    split2 = zip(*trial_ticks)

    x1 = alpha[0]  # timestamps
    y1 = alpha[1]  # alpha waves values
    ...
    x3 = beta_data[0]  # timestamps
    y3 = beta_data[1]  # alpha waves values

    plt.figure(1)
    plt.subplot(211)

    plt.plot(x1, y1, 'k', x2, y2, 'bo', x3,y3)
    # plt.plot(x1,y1)
    plt.show()
