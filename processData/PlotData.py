...
with open('/Users/shiran/out/16-12-10_19-58/Alpha.csv', 'rU') as csvfile:
    reader = csv.reader(csvfile, delimiter=csv.excel.delimiter)
    numrows = sum(1 for row in csvfile)


def appedEEGOneByOne():
    global xs_time
    xs_time = []
    eeg = np.empty((numrows, 4))
    alpha = []
    beta = []
    gamma = []
    delta = []
...
with open('/Users/shiran/out/16-12-10_19-58/Alpha.csv', 'rU') as csvfile:
    ...
    eeg_3 = []
    xs_time = []
    appedEEGOneByOne(eeg_0, eeg_1, eeg_2, eeg_3, xs_time)
